...
$(function(){
   ...
   $('#btnForm').click(function(){
      location.href = '${pageContext.request.contextPath}/empl/dy_recsroom/dy_recsroomForm.do?prjct_id='+prjct_id;
   });
   $('table tr:gt(0)').click(function(){
	   var dynm_recsroom_id = $(this).find('td:eq(0) input').val();
	   $(location).attr('href', '${pageContext.request.contextPath}/empl/dy_recsroom/dy_recsroomView.do?dynm_recsroom_id=' + dynm_recsroom_id + '&prjct_id='+prjct_id);
   });
});
</script>
</head>
                            ...
                            <thead class="bg-teal bg-lighten-4">
                                <tr>
									...
									<tbody>
										...
										<c:forEach items="${recsRoomList}" var="frb">
											<tr>
												<td><input type="hidden" value="${frb.dynm_recsroom_id }">${frb.rnum}</td>
												<td>${frb.dynm_recsroom_nm }</td>
												<td>${frb.dynm_recsroom_id }</td>
												<td>${frb.dynm_recsroom_empl }</td>
												<td>${frb.dynm_recsroom_date }</td>
												<td>${frb.dynm_recsroom_inquiry }</td>
											</tr>
										</c:forEach>
