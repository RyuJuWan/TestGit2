...
public class LogPrintUtil {
	...
	// Advice(Aspect가 PointCut에 Weaving되는 시점) : 메서드 호출 전. 
	//											    메서드 종료 후. 
	//											    메서드 호출 후 익셉션 발생 후.
	//											    메서드 호출 후과 종료 후.
	public void theWholeJoinpointCallBefore(JoinPoint joinPoint){
		// 메서드 호출 전.
		// 해당 joinPoint를 포함하는 빈 클래스의 패키지명.클래스명 반환
		String clazzName = joinPoint.getTarget().getClass().getName();
		// 해당 joinPoint(메서드명) 반환
		String joinPointMethod = joinPoint.getSignature().getName();
		
		logger.debug("clazzName : {} | methodName : {}가 호출되었습니다.", clazzName, joinPointMethod);
	}
