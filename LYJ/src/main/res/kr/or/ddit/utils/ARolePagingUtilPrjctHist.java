...
public class ARolePagingUtilPrjctHist {
	...
	public ARolePagingUtilPrjctHist(int totalCount, int prjctHistCurrentPage, HttpServletRequest request) {
		super();
	...
	public ARolePagingUtilPrjctHist(int totalCount, int prjctHistCurrentPage, HttpServletRequest request, int blockCount, int blockPage, String empl_id) {
		super();
		this.totalCount = totalCount;
		this.prjctHistCurrentPage = prjctHistCurrentPage;
		this.request = request;
...
//		}
		...
		if((this.prjctHistCurrentPage-1)==0){
		    ...
		    this.pageHtmls.append("<li class='page-item'><a class='page-link' href='"+ requestURI+"?"+params+ "prjctHistCurrentPage="+(this.prjctHistCurrentPage-1) +"&empl_id=" + empl_id + "' aria-label='Previous'><span aria-hidden='true'>« Prev</span><span class='sr-only'>Previous</span></a></li>");
		}
		...
		for(int i=this.startPage; i<=this.endPage; i++){
			if(this.prjctHistCurrentPage == i){
				...
				this.pageHtmls.append("<li class='page-item'><a class='page-link' href='" + requestURI + "?" + params + "prjctHistCurrentPage=" + i + "&empl_id=" + empl_id + "'>" + i + "</a></li>");
			}
		}
		
		// 다음
		if(this.prjctHistCurrentPage < this.totalPage){
			this.pageHtmls.append("<li class='page-item'><a class='page-link' href='" + requestURI + "?" + params + "prjctHistCurrentPage=" + (this.prjctHistCurrentPage + 1) + "&empl_id=" + empl_id + "' aria-label='Next'><span aria-hidden='true'>Next »</span><span class='sr-only'>Next</span></a></li>");
		}else{
