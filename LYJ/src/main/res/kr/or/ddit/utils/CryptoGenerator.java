...
public class CryptoGenerator {
	...
	public Map<String, String> getGeneratePairKey(HttpSession session){
		...
		try {
			...
			RSAPublicKeySpec publicKeySpec = (RSAPublicKeySpec)factory.getKeySpec(publicKey, RSAPublicKeySpec.class);
			// 단정도(32bit-float), 배정도(64bit-double)
			// -143.124378432954360852
			// 단정도(32bit-float) 2진수 : 부호비트(양수:0|음수:1) + 지수(소숫점 위치 8bit) + 가수부(실수값 표현 23bit)
			// 배정도(64bit-double) 2진수 : 부호비트(양수:0|음수:1) + 지수(소숫점 위치 11bit) + 가수부(실수값 표현 52bit)
